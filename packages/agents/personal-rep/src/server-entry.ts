...
const facts = buildAgentFacts({
  id: did,
  agentName,
  label: "Citizen's Personal Representative",
  description: 'Autonomously navigates government and vendor agents to achieve citizen goals.',
...
app.log.info({ did: manager.did, publicKey: publicKeyToBase64url(manager.publicKey) }, 'personal-rep started');

await manager.registerFactsAndIndex(facts);
app.log.info({ did: manager.did }, 'personal-rep registered');
