...
const PORT = parseInt(process.env.PORT ?? '8466', 10);
...
const facts = buildAgentFacts({
  ...
  description: 'Conducts food truck health inspections and issues signed approval credentials.',
...
await configureApp(app, manager, async (a) => {
  await a.register(registerRoutes, { manager, foodTruckVendorDid });
});

await app.listen({ port: PORT, host: '0.0.0.0' });
app.log.info({ did: manager.did, publicKey: publicKeyToBase64url(manager.publicKey) }, 'gov-health-dept started');

await manager.registerFactsAndIndex(facts);
app.log.info({ did: manager.did }, 'gov-health-dept registered');
