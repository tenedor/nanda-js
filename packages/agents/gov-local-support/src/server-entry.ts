...
const PORT = parseInt(process.env.PORT ?? '8464', 10);
...
const parkingDeptDid = process.env.PARKING_DEPT_DID;
...
if (!parkingDeptDid) throw new Error('PARKING_DEPT_DID is required');
if (!foodTruckVendorDid) throw new Error('FOOD_TRUCK_VENDOR_DID is required');

const facts = buildAgentFacts({
  id: did,
  agentName,
  label: 'Government Local Business Support',
  description: 'Advises citizens on starting a food truck business and provides contacts for required agencies.',
...
await configureApp(app, manager, async (a) => {
  await a.register(registerRoutes, { businessLicensingDid, parkingDeptDid, foodTruckVendorDid });
});

await app.listen({ port: PORT, host: '0.0.0.0' });
app.log.info({ did: manager.did, publicKey: publicKeyToBase64url(manager.publicKey) }, 'gov-local-support started');

await manager.registerFactsAndIndex(facts);
app.log.info({ did: manager.did }, 'gov-local-support registered');
