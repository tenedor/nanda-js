...
const PORT = parseInt(process.env.PORT ?? '8465', 10);
...
const healthDeptDid = process.env.HEALTH_DEPT_DID;
...
if (!healthDeptDid) throw new Error('HEALTH_DEPT_DID is required');

const facts = buildAgentFacts({
  id: did,
  agentName,
  label: 'Government Business Licensing Office',
  description: 'Issues business registrations, provisional and final restaurant licenses.',
...
await configureApp(app, manager, async (a) => {
  await a.register(registerRoutes, { manager, healthDeptDid });
});

await app.listen({ port: PORT, host: '0.0.0.0' });
app.log.info({ did: manager.did, publicKey: publicKeyToBase64url(manager.publicKey) }, 'gov-business-licensing started');

await manager.registerFactsAndIndex(facts);
app.log.info({ did: manager.did }, 'gov-business-licensing registered');
