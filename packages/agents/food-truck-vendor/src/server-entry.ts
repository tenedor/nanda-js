...
const PORT = parseInt(process.env.PORT ?? '8468', 10);
...
const facts = buildAgentFacts({
  id: did,
  agentName,
  label: 'Food Truck Vendor',
  description: 'Rents food trucks to registered businesses.',
  version: PROTOCOL_VERSION,
  providerName: 'NANDA Food Trucks LLC',
...
await configureApp(app, manager, async (a) => {
  await a.register(registerRoutes, { manager, businessLicensingDid });
});

await app.listen({ port: PORT, host: '0.0.0.0' });
app.log.info({ did: manager.did, publicKey: publicKeyToBase64url(manager.publicKey) }, 'food-truck-vendor started');

await manager.registerFactsAndIndex(facts);
app.log.info({ did: manager.did }, 'food-truck-vendor registered');
